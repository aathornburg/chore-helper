  ...
  return (
    <div className="family-page">
      <header className="workspace-hero compact-hero">
        <div>
          <p className="eyebrow">Shared household access</p>
          <h1>Family</h1>
          <p className="lede">
            Invite household members and decide who can manage the shared chore plan.
          </p>
        </div>
      </header>
...
function FamilyHouseholdPanel({ currentUserId, household }: FamilyPanelProps) {
  ...
  return (
    <section className="family-household panel" aria-label={`${household.name} family access`}>
        ...
        <div className="family-panel-grid">
          <section aria-labelledby={`${household.id}-members`}>
            ...
            <ul className="family-list">
              {members.map((member) => (
                <li className="family-row" key={member.userId}>
                  <div>
                    <strong>{memberLabel(member)}</strong>
                    <span>{member.primaryEmail && member.displayName ? member.primaryEmail : roleLabel(member.role)}</span>
                  </div>
          ...
          <section aria-labelledby={`${household.id}-invitations`}>
            <div className="section-heading compact-heading">
              <h3 id={`${household.id}-invitations`}>Invitations</h3>
            ...
            <ul className="family-list invitation-list">
              {invitations.map((invitation) => (
                <li className="family-row" key={invitation.id}>
                  <div>
              ...
              {invitations.length === 0 ? <li className="empty-state">No invitations sent.</li> : null}
            </ul>
          </section>
        </div>
      ) : null}
